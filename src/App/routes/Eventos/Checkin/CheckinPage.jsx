...
import {
  ...
  RefreshCw, Search, X, Lock, ClipboardCheck, Package, Boxes,
} from 'lucide-react';
import {
...
const STATIONS = [
  { id: 1, label: 'Estación 1', sublabel: 'Check-in · Uniforme',          Icon: ClipboardCheck, activeClass: 'bg-emerald-600 text-white shadow-sm', inactiveClass: 'text-foreground hover:bg-muted', lockedClass: 'text-muted-foreground/40 cursor-not-allowed' },
  { id: 2, label: 'Estación 2', sublabel: 'Maleta · Almuerzo · Refrigerio', Icon: Package,        activeClass: 'bg-indigo-600 text-white shadow-sm',  inactiveClass: 'text-foreground hover:bg-muted', lockedClass: 'text-muted-foreground/40 cursor-not-allowed' },
  { id: 3, label: 'Estación 3', sublabel: 'Insumos',                        Icon: Boxes,          activeClass: 'bg-[#DD7419] text-white shadow-sm',   inactiveClass: 'text-foreground hover:bg-muted', lockedClass: 'text-muted-foreground/40 cursor-not-allowed' },
];
      ...
      <div className="shrink-0 bg-white border-b border-border px-4">
        <div className="flex gap-1 py-2">
          {STATIONS.map(({ id, label, sublabel, Icon, activeClass, inactiveClass, lockedClass }) => {
            ...
            return (
              <button key={id}
                onClick={() => accessible && !isComingSoon && handleStationChange(id)}
                ...
                className={`relative flex-1 flex flex-col items-center gap-0.5 px-2 py-2 rounded-lg text-xs font-semibold transition-all ${
                  !accessible || isComingSoon ? lockedClass : isActive ? activeClass : inactiveClass
                }`}
              >
                ...
                <span className={`text-[10px] font-normal leading-tight text-center ${isActive ? 'text-white/80' : 'text-muted-foreground'}`}>
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-4 space-y-4">
          ...
          {activeStation <= 2 && (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {/* Limpiar */}
              {hasActiveFilters && (
                <Button variant="outline" onClick={handleClearFilters} className="h-9 text-muted-foreground shrink-0">
                  <X className="w-3.5 h-3.5" />
                </Button>
              )}

              {/* Filtro Todos / Pendientes / Listos */}
              <div className="flex border border-border rounded-md overflow-hidden shrink-0">
                {[['all', 'Todos'], ['pending', 'Pendientes'], ['done', 'Listos']].map(([val, lbl]) => (
                  <button key={val} onClick={() => { setStatusFilter(val); setCurrentPage(1); }}
                    className={`px-3 py-1.5 text-xs font-medium transition ${statusFilter === val ? 'bg-[#234465] text-white' : 'bg-white text-foreground hover:bg-muted'}`}
                  >{lbl}</button>
                ))}
              </div>
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  ...
                  .map((item, idx) => item === '...' ? (
                    ...
                    <button key={item} onClick={() => setCurrentPage(item)}
                      className={`h-8 min-w-8 px-2 rounded-md text-xs font-medium transition ${safePage === item ? 'bg-[#234465] text-white' : 'border border-border text-foreground hover:bg-muted'}`}
                    >{item}</button>
                  ))}
