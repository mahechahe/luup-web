...
import { getInventoryListService } from '../../services/inventoryServices';
...
/* ── Modal principal ─────────────────────────────────────── */
export function AddInventoryModal({ open, onOpenChange, collab, eventId = null, onAdded }) {
  ...
  const breakdownOk = pendientes >= 0 && (usados + danados + devueltos + pendientes === totalQty);
  ...
  useEffect(() => {
    if (!open) return;
    setLoading(true);
    getInventoryListService({ page: currentPage, nombre: search || undefined }).then((res) => {
      if (res.status) {
        setItems(res.items);
  ...
  const handleAddToStaged = () => {
    ...
    setStaged((prev) => {
      ...
      return [...prev, { item: selectedItem, totalQty, usados, danados, devueltos, pendientes }];
    });
    setSelectedItem(null);
    setTotalQty(1);
    setUsados(0);
  ...
  const handleConfirm = async () => {
    ...
    setSaving(true);
    try {
      // TODO: reemplazar con servicio real de agregar inventario post-confirmación
      // Por ahora llama al servicio existente por cada ítem staged
      const { assignInventoryToCollaboratorService } = await import('../../services/inventoryServices');
      let allOk = true;
      for (const s of staged) {
        const res = await assignInventoryToCollaboratorService({
          ...
          quantity: s.totalQty,
          usedQuantity: s.usados,
          damagedQuantity: s.danados,
          returnedQuantity: s.devueltos,
          pendingQuantity: s.pendientes,
        ...
        <div className="flex-1 overflow-y-auto flex flex-col">
            ...
            <table className="w-full text-xs">
              <thead className="sticky top-0 z-10">
                <tr className="bg-muted/70 border-b border-border">
                  ...
                  <th className="text-center font-semibold text-muted-foreground px-3 py-2 w-16">Stock</th>
                ...
                {loading ? (
                  Array.from({ length: 4 }).map((_, i) => (
                    <tr key={i} className="border-b border-border/50 animate-pulse">
                      <td className="px-3 py-2"><div className="h-3 bg-muted rounded-full w-40" /></td>
                      <td className="px-3 py-2"><div className="h-4 bg-muted rounded-full w-10 mx-auto" /></td>
                      <td />
                    </tr>
                  ))
                ) : items.length === 0 ? (
                  ...
                  items.map((item, idx) => (
                    <tr
                      ...
                      className={`border-b border-border/50 transition-colors ${
                        selectedItem?.id === item.id ? 'bg-[#DD7419]/8' : 'hover:bg-muted/30'
                      }`}
                    >
          ...
          <div className="border-t border-border px-5 py-2 flex items-center justify-between gap-3 shrink-0">
            <p className="text-xs text-muted-foreground">
              {loading ? '—' : <><span className="font-semibold text-foreground">{total === 0 ? 0 : startIdx + 1}–{Math.min(startIdx + pagination.limit, total)}</span> de <span className="font-semibold text-foreground">{total}</span></>}
            </p>
            <div className="flex items-center gap-1">
              <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={safePage === 1 || loading} className="h-7 w-7 rounded-md border border-border flex items-center justify-center text-muted-foreground hover:bg-muted disabled:opacity-40 disabled:cursor-not-allowed transition">
                <ChevronLeft className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={safePage === totalPages || loading} className="h-7 w-7 rounded-md border border-border flex items-center justify-center text-muted-foreground hover:bg-muted disabled:opacity-40 disabled:cursor-not-allowed transition">
            ...
            <div className="border-t-2 border-[#DD7419]/20 bg-[#DD7419]/5 px-5 py-4 shrink-0 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold text-foreground truncate">{selectedItem.nombre}</p>
                  <p className="text-[10px] text-muted-foreground">
                    Stock disponible: <span className="font-semibold text-[#DD7419]">{selectedItem.cantidad}</span>
                  </p>
                </div>
                <button onClick={() => setSelectedItem(null)} className="text-muted-foreground hover:text-foreground transition">
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between gap-3 bg-background rounded-xl px-4 py-3 border border-border">
                  <div>
                    <p className="text-xs font-bold text-foreground">Total a asignar</p>
                    <p className="text-[10px] text-muted-foreground">La suma de estados debe ser igual a este valor</p>
                  </div>
                  <div className="flex items-center border border-[#DD7419]/40 rounded-lg overflow-hidden shrink-0">
                    <button
                      type="button"
                      onClick={() => { const v = clamp(totalQty - 1, 1, selectedItem.cantidad); setTotalQty(v); setUsados((u) => clamp(u, 0, v)); setDanados((d) => clamp(d, 0, v)); setDevueltos((dv) => clamp(dv, 0, v)); }}
                      disabled={totalQty <= 1}
                    ...
                    >−</button>
                    <span className="w-10 text-center text-sm font-bold text-foreground select-none">{totalQty}</span>
                    <button
                      type="button"
                      onClick={() => setTotalQty((q) => clamp(q + 1, 1, selectedItem.cantidad))}
                      disabled={totalQty >= selectedItem.cantidad}
                      className="w-9 h-9 flex items-center justify-center text-[#DD7419] font-bold hover:bg-[#DD7419]/10 disabled:opacity-30 transition"
                    >+</button>
                  </div>
                </div>
                ...
                <div className="bg-background rounded-xl px-4 py-3 border border-border space-y-3">
                  <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide">Distribución del estado</p>
                  <div className="grid grid-cols-4 gap-2">
                    <NumInput label="Usados" value={usados} onChange={setUsados} max={totalQty - danados - devueltos} color="text-[#234465] dark:text-[#7493B2]" />
                    <NumInput label="Dañados" value={danados} onChange={setDanados} max={totalQty - usados - devueltos} color="text-destructive" />
                    <NumInput label="Devueltos" value={devueltos} onChange={setDevueltos} max={totalQty - usados - danados} color="text-emerald-600" />
                    <div className="flex flex-col items-center gap-1.5">
                      <span className="text-[10px] font-semibold uppercase tracking-wide text-amber-600">Pendientes</span>
                      <div className={`w-full h-8 flex items-center justify-center rounded-lg text-sm font-bold ${
                        pendientes < 0 ? 'bg-destructive/10 text-destructive' : 'bg-amber-50 dark:bg-amber-900/20 text-amber-600'
                      }`}>
                        {pendientes}
                      </div>
                    </div>
                  ...
                  {pendientes < 0 && (
                    <p className="text-[11px] text-destructive font-medium">
                      La suma supera el total asignado ({totalQty}). Ajusta los valores.
                    </p>
                  )}
                </div>
              </div>
          ...
          {staged.length > 0 && (
            <div className="px-5 py-4 border-t border-border space-y-2">
              ...
              <div className="space-y-2">
                {staged.map((s) => (
                  <div key={s.item.id} className="bg-muted/40 rounded-xl px-3 py-2.5 space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <Package className="w-3.5 h-3.5 text-[#DD7419] shrink-0" />
                        <p className="text-xs font-semibold text-foreground truncate">{s.item.nombre}</p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-[11px] font-bold text-[#DD7419] bg-[#DD7419]/10 px-2 py-0.5 rounded-full">
                          Total: {s.totalQty}
                        </span>
                        <button onClick={() => handleRemoveStaged(s.item.id)} className="text-muted-foreground hover:text-destructive transition">
                    ...
                    <div className="grid grid-cols-4 gap-1.5 text-center">
                      {[
                        { label: 'Usados', value: s.usados, color: 'text-[#234465] dark:text-[#7493B2]' },
                        { label: 'Dañados', value: s.danados, color: s.danados > 0 ? 'text-destructive' : 'text-muted-foreground' },
                        { label: 'Devueltos', value: s.devueltos, color: 'text-emerald-600' },
                        { label: 'Pendientes', value: s.pendientes, color: 'text-amber-600' },
                      ].map(({ label, value, color }) => (
                        <div key={label} className="bg-background rounded-lg py-1.5">
        ...
        <div className="border-t border-border px-5 py-3 shrink-0 flex gap-2">
          ...
          >
            {saving ? (
              ...
              <>Confirmar {staged.length > 0 && `(${staged.length} ${staged.length === 1 ? 'ítem' : 'ítems'})`}</>
            )}
          </button>
        </div>
      </DialogContent>
